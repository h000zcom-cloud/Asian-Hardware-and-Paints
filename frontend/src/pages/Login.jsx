...
export default function Login({ onLogin }) {
  ...
  const submit = async e => { e.preventDefault(); setError(''); setBusy(true); try { const { data } = await api.post('/auth/login', { username, password }); if (data.token) { localStorage.setItem('ah_token', data.token); } onLogin(data); navigate('/admin'); } catch (err) { setError(errorText(err)); } finally { setBusy(false); } };
